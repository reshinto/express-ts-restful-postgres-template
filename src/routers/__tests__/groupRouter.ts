...
import setupPassport from "../../loaders/setupPassport";
...
describe("/groups", () => {
  ...
  it("should return status code 403 when auth token has invalid signature", async () => {
    await setupPassport();
    const token =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6Ik1heCIsImlhdCI6MTYzOTE3ODg5MCwiZXhwIjoxNjM5MTgwNjkwfQ.-C-4mfhAiLuo3I9TBUaAwEQ97FQFmTPpwVoDWZd74df";
    const res = await request(app)
      .get("/groups")
      .set("Authorization", `bearer ${token}`);
...
describe("/groups/:id", () => {
  ...
  it("should return status code 401 when auth token is not provided", async () => {
    ...
    const res = await request(app).put(`/groups/${id}`).send(payload);
    const expected = "Error: No auth token";
...
describe("/groups/:id", () => {
  ...
  it("should return status code 401 when auth token is not provided", async () => {
    ...
    const res = await request(app).post("/groups/create").send(payload);
    const expected = "Error: No auth token";
